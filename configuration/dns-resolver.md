...
This is the preferred workaround because it is system-wide, works better with Windows-native networking once the Warden root CA is trusted, and may succeed on systems where plain `127.0.0.1` DNS is disrupted by Hyper-V, `SharedAccess`, or similar Windows networking behavior.
   ...
   * `Alternate DNS`: your previous/preferred resolver if you want internet access to keep working when Warden is stopped, or leave it empty if you want Windows to depend only on Warden
   ...
   `Fallback to plaintext` did not restore DNS when Warden was stopped. A working `Alternate DNS` server was what kept normal internet DNS lookups working while Warden was offline. When `Alternate DNS` was left empty, both Warden `*.test` domains and normal public DNS lookups failed until Warden came back or the DNS settings were changed.
